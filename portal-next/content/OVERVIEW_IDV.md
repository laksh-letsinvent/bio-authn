...
For the authenticity track I also ran a separate external validation against SIDTD — real identity document templates with ten distinct forgery types (photo substitution, field overwrite, font change, cropping, and others) covering ten countries. The SIDTD images are not generated by me; they come from a published academic dataset under CC BY-SA 2.5. That run is reported separately in Results and is the only part of this work that uses documents from outside my own corpus.

## The stack

Same engine as "Face Value": the ONNX face model for the match, Claude through its command-line mode for the vision calls, and the same eval harness and cost accounting. For the extraction baseline, Tesseract 5 via pytesseract — run on the full document image without layout pre-segmentation, which is deliberately the weak version (that's what VLM is being compared against). "Hard Copy" is its own front end in burgundy on its own address, but the instrument is shared. One pipeline, two prototypes.
